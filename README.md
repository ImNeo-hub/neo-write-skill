...
```bash
git clone <this-repo> ~/.claude/skills/shuchu
```

首次校准（必做，否则写出来还是 AI 腔）：

```bash
cp ~/.claude/skills/shuchu/memory/voice-profile.template.md \
   ~/.claude/skills/shuchu/memory/voice-profile.md
```

然后在 Claude Code 里说 `/输出`，它会带你跑「冷启动校准」——你贴几篇自己满意的旧文，它抽出你的声音特征填进档案。
